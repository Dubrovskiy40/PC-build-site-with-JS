...
const updateTimer = () => {
    ...
    daysBlock.nextElementSibling.textContent = numWord(days, daysArr);

    const hoursArr = ['час', 'часа', 'часов'];
    hoursBlock.nextElementSibling.textContent = numWord(hours, hoursArr);

    const minutesArr = ['минута', 'минуты', 'минут'];
    minutesBlock.nextElementSibling.textContent = numWord(minutes, minutesArr);

    const secondsArr = ['секунда', 'секунды', 'секунд'];
    secondsBlock.nextElementSibling.textContent = numWord(seconds, secondsArr);
